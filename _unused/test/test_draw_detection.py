...
4. Replay Game from History - Replays a specific game from history to verify repetition detection
...
def test_replay_game_from_history(moves_san: list, game_num: int = 1):
    """Test repetition detection by replaying a game from history."""
    print("\n" + "="*70)
    print(f"TEST 4: Replay Game from History (Game {game_num})")
    print("="*70)
    ...
    print(f"Replaying {len(moves_san)} moves from game history...")
    print("-" * 70)
    ...
    for i, move_san in enumerate(moves_san):
        ...
        try:
            ...
            print(f"Error parsing move {i+1}: {move_san} ({e})")
            break
        ...
        can_claim_3fold = False
        is_5fold = False
        if hasattr(env.board, 'can_claim_threefold_repetition'):
            try:
                can_claim_3fold = env.board.can_claim_threefold_repetition()
            except:
                pass
        if hasattr(env.board, 'is_fivefold_repetition'):
            try:
                is_5fold = env.board.is_fivefold_repetition()
            except:
                pass
        ...
        if is_game_over:
            outcome = env.board.outcome(claim_draw=True)
            if outcome:
                termination_reason = outcome.termination.name if hasattr(outcome.termination, 'name') else str(outcome.termination)
    ...
    if repetition_detected:
        print("\n✓ SUCCESS: Repetition correctly detected when replaying game!")
        return True
    elif sum(1 for c in position_counts.values() if c >= 3) > 0:
        print("\n⚠ WARNING: Repetition occurred but was not detected!")
        return False
    else:
        print("\n✓ Game replayed successfully (no repetition in this game)")
        return True
...
def main():
    ...
    # Test 4: Replay game from history (optional - only if moves provided)
    # This test can be enabled by uncommenting and providing moves
    # try:
    ...
    #     result = test_replay_game_from_history(game_moves, game_num=1)
    #     results.append(("Replay Game from History", result))
    # except Exception as e:
    #     print(f"\n✗ Test 4 failed with error: {e}")
    #     import traceback
    #     traceback.print_exc()
    #     results.append(("Replay Game from History", False))
