...
def inference_server_worker(
    ...
    try:
        ...
        # Verify CUDA is available when using cuda device (common cause of silent failure in spawn)
        if "cuda" in (device_str or "").lower() and not torch.cuda.is_available():
            _log_err(
                f"InferenceServer: CUDA requested ({device_str}) but torch.cuda.is_available() is False. "
                "Check that you are on a GPU kernel (e.g. Kaggle GPU or TPU+GPU)."
            )
            if ready_event is not None:
                ready_event.set()  # Signal so main process does not hang
            return
        ...
        if initial_state_dict is not None:
            try:
                ...
                if logging_enabled:
                    logger.warning(f"InferenceServer: load_state_dict failed: {e}")
        ...
        _log_err(f"InferenceServer: Fatal error: {e}\n{traceback.format_exc()}")
        if ready_event is not None:
            ready_event.set()
        raise
